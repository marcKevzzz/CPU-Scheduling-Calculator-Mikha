...
export function renderResultTableTurnaround(result) {
  ...
  result.forEach((r) => {
    const row = `
      ...
      <tr>
                  <td class="border px-7 text-center py-2 font-semibold jobs">TT${process++}</td>
                  <td class="border px-2 py-2 w-[9rem]">
  ...
  const ttave = `<tr>
  <td class="border px-7 text-center py-2 font-semibold jobs">TTave</td>
  <td class="border px-2 py-2 w-[9rem]">${ave}  /   ${process - 1}  =   ${(
    ave /
    (process - 1)
  ).toFixed(2)} ms</td>
  </tr>`;
...
export function renderResultTableWaiting(result) {
  ...
  result.forEach((r) => {
    const row = `
                  ...
                  <tr>
                  <td class="border px-7 text-center py-2 font-semibold jobs">WT${process++}</td>
                  <td class="border px-2 py-2 w-[9rem]">
  ...
  const ttave = `<tr>
  <td class="border px-7 text-center py-2 font-semibold jobs">WTave</td>
  <td class="border px-2 py-2 w-[9rem]">${ave}  /   ${process - 1}  =   ${(
    ave /
    (process - 1)
  ).toFixed(2)} ms</td>
...
export function renderTableHeader(tableSelector, algorithm) {
  ...
  let headerHTML = `
      <tr>
       <th class="border px-2 py-1 text-xs md:text-sm">JOBS</th>
                 <th class="border px-2 py-1 text-xs md:text-sm">ARRIVAL TIME</th>
                 <th class="border px-2 py-1 text-xs md:text-sm">BURST TIME</th>
    `;
  //   if (algorithm == "RR") {
  //     headerHTML += `<th>
  ...
  if (algorithm === "PP" || algorithm === "NPP") {
    headerHTML += ` <th class="border px-2 py-1 text-xs md:text-sm">PRIORITY</th>`;
  }
...
export function addRow(tableSelector, algorithm = "", isFirstRow = false) {
  ...
  let rowContent = `
    <td class="border px-7 font-semibold jobs">
               J${processCounter++}
              </td>
  ...
  if (algorithm === "RR" && isFirstRow) {
    rowContent += `
       ...
       <td class="border" class="timeQuantum-col">
                <input
                  class="input border-0 outline-0 w-full bg-transparent drop-shadow-none ps-1 ring-0 input py-2"
  ...
  if (algorithm === "NPP" || algorithm === "PP") {
    rowContent += `
       <td class="border" class="priority-col">
                <input
