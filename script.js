...
export function updateTableColumns(selectedValue) {
  ...
  if (needsPriority && !hasPriorityColumn) {
    ...
    priorityHeader.innerHTML = `   <th class="border px-2 py-1 text-xs md:text-sm">PRIORITY</th>`;
    headerRow.appendChild(priorityHeader);

    bodyRows.forEach((row) => {
      const newCell = document.createElement("td");
      newCell.classList.add("priority-col", "border");
  ...
  if (needsTimeQuantum && !hasTimeQuantumColumn) {
    ...
    tqHeader.innerHTML = `TIME QUANTUM`;
...
document.addEventListener("DOMContentLoaded", function () {
  document
    .getElementById("algorithm-buttons")
    .addEventListener("click", function (e) {
      if (e.target && e.target.matches("button.btn-option")) {
  ...
  if (clearBtn) {
    clearBtn.addEventListener("click", () => {
      ...
      document.getElementById(
        "gbody"
      ).innerHTML = `  <div class="grid grid-cols-6 gap-0 mb-6 sm:w-[25em] w-[15em] mx-auto">
